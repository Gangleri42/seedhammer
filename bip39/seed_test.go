...
// The official BIP39 English vectors, whose seeds are all salted with
// the passphrase "TREZOR". Fetched from trezor/python-mnemonic, not
// transcribed: MnemonicSeed's passphrase argument had no coverage at
// all before this, and it decides which wallet a phrase opens.
//
// Every vector is ASCII, which is the whole domain MnemonicSeed
// promises. See its doc comment for why normalisation is absent.
...
func TestMnemonicSeedVectors(t *testing.T) {
	for _, v := range seedVectors {
		...
		got := hex.EncodeToString(MnemonicSeed(m, testVectorPassphrase))
		if got != v.seed {
			t.Errorf("seed for %q\n got %s\nwant %s", v.mnemonic, got, v.seed)
		}
	}
}
